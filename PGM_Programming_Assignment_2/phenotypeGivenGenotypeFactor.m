...
alphas = alphaList(assignments(:, 2));
rows_trait =  alphas > 0.5;
trait = assignments(:,1) == 1;

% calculate xnor to give resultant truth table
phenotypeFactor.val = ~xor(trait, rows_trait);

% calculate probabilities
idx = find(assignments(:,1) == 1);
temp = zeros(phenotypeFactor.card, 1);

% assign probabilities where the trait is present and probability is positive
temp(idx) = alphas(idx);

% assign inverse probability when trait is not present 
temp(setdiff((1:6), idx)) = 1 - alphas(idx);
