...
for i = 1:size(E, 1),
    ...
    for j = 1:length(F),
        ...
        if (~isempty(indx)),
            ...
            % get rows which have variable !equal to observed value. NOTE: get index of variable in factor first
            rows_to_ignore = find(assignments(:,F(j).var == v) != x);
            
            % get assignments to set to zero
            assignments_to_null = IndexToAssignment(rows_to_ignore, F(j).card);
            
            % set assignments to zero
            F(j) = SetValueOfAssignment(F(j), assignments_to_null, 0);
